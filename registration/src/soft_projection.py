...
class SoftProjection(nn.Module):
    ...
    def _group_points(self, point_cloud, query_cloud, point_features=None):
        ...
        grouped_points = group_point(
            point_cloud, idx)  # B x 3 x QueryPoints x K
        grouped_features = (
            None if point_features is None else group_point(
                point_features, idx)
        )  # B x F x QueryPoints x K
        return grouped_points, grouped_features

    def _get_distances(self, grouped_points, query_cloud):
        deltas = grouped_points - \
            query_cloud.unsqueeze(-1).expand_as(grouped_points)
        dist = torch.sum(deltas ** 2, dim=_axis_to_dim(3),
                         keepdim=True) / self.sigma()
        return dist
...
if __name__ == "__main__":
    ...
    point_features_pl = (
        torch.tensor(point_features, dtype=torch.float32).permute(
            0, 2, 1).cuda()
    )
    query_cloud_pl = (
    ...
    mse_points = np.mean(
        np.sum((projected_points - expected_nn_cloud) ** 2, axis=1))
